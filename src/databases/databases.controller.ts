...
export class DatabasesController {
  ...
  @Get(':id/:item')
  async dbItem(@Param('id') id: string, @Param('item') item, @Res() response) {
    try {
      const result = await this.databasesService.getItemFromKeyValue(id, item);
  ...
  async dbPut(
    ...
    try {
      const result = await this.databasesService.addItemToKeyValue(id, body);
      response.json(result);
    } catch (e) {
      response.status(500).json({ error: e.message });
    }
